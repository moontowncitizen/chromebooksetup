...
import FilterEngine, { ENGINE_VERSION } from '../npm/@cliqz/adblocker/dist/esm/engine/engine.js';
...
import { getLinesWithFilters, mergeDiffs } from '../npm/@cliqz/adblocker/dist/esm/lists.js';
    ...
    const response = await fetch(
      chrome.runtime.getURL(
        `rule_resources/engine-${name}${
          "chromium" === 'firefox' || name === 'trackerdb' ? '' : '-cosmetics'
        }.dat`,
      ),
    );
...
function addChangeListener(name, fn) {
...
async function update(name) {
  ...
  try {
    const urlName =
      name === 'trackerdb'
        ? 'trackerdbMv3'
        : `dnr${"chromium" === 'firefox' ? '' : '-cosmetics'}-${name}`;
...
function replace(name, engineOrEngines) {
  ...
  const engine = engines.length > 1 ? FilterEngine.merge(engines) : engines[0];
...
export { CUSTOM_ENGINE, FIXES_ENGINE, MAIN_ENGINE, TRACKERDB_ENGINE, addChangeListener, create, get, init, remove, replace, setEnv, update };
